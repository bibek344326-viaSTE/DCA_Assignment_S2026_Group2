...
public class CommandDispatcherTests
{
    ...
    [Fact]
    public async Task
        CommandDispatcher_WhenCommandIsDispatched_AndMultipleHandlersAreRegistered_DispatchesThroughCorrectHandler()
    {
        // Arrange
        ServiceCollection serviceCollection = new ServiceCollection();
        serviceCollection.AddScoped<ICommandHandler<CreateEventCommand>, TestCreateEventCommandHandler>();
        ...
        TestCreateEventCommandHandler? createEventCommandHandler =
            serviceProvider.GetRequiredService<ICommandHandler<CreateEventCommand>>() as TestCreateEventCommandHandler;
        TestUpdateTitleCommandHandler? updateTitleCommandHandler =
            serviceProvider.GetRequiredService<ICommandHandler<UpdateTitleCommand>>() as
                TestUpdateTitleCommandHandler;
        Assert.NotNull(createEventCommandHandler);
        Assert.Equal(1, createEventCommandHandler.CallCount);

        Assert.NotNull(updateTitleCommandHandler);
        Assert.Equal(0, updateTitleCommandHandler.CallCount);
    }
        ...
        CommandDispatcher_WhenCommandIsDispatched_AndMultipleHandlersAreRegistered_MultipleCommandsAreDispatched_DispatchesThroughCorrectHandlers()
    {
        ...
        UpdateTitleCommand sampleCommand2 = UpdateTitleCommand.Create(Guid.NewGuid(), "New Title").Payload!;
        ...
        Assert.NotNull(updateTitleCommandHandler);
        Assert.Equal(1, updateTitleCommandHandler.CallCount);
    }
