...
[Route("api/events/{eventId:guid}/invitations/accept")]
public sealed class AcceptInvitationEndpoint(IDispatcher dispatcher)
    : ApiEndpoint<GuestEmailRequest, object>
{
    ...
    public override async Task<ActionResult<object>> HandleAsync([FromBody] GuestEmailRequest request)
        => await CommandEndpoint.DispatchAsync(this, dispatcher,
            AcceptInvitationCommand.Create(RouteValueReader.Guid(this, "eventId"), request.GuestEmail));
}
