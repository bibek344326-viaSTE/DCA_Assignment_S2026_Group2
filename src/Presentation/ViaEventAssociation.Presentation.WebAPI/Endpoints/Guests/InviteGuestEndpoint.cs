...
[Route("api/events/{eventId:guid}/invitations")]
public sealed class InviteGuestEndpoint(IDispatcher dispatcher)
    : ApiEndpoint<GuestEmailRequest, object>
{
    ...
    public override async Task<ActionResult<object>> HandleAsync([FromBody] GuestEmailRequest request)
        => await CommandEndpoint.DispatchAsync(this, dispatcher,
            InviteGuestCommand.Create(RouteValueReader.Guid(this, "eventId"), request.GuestEmail));
}
