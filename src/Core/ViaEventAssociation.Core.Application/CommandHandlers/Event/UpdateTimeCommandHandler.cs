...
public class UpdateTimeCommandHandler(IEventRepository eventRepository, IUnitOfWork unitOfWork)
    : ICommandHandler<UpdateTimeCommand>
{
    public async Task<Result> HandleAsync(UpdateTimeCommand command)
    {
        ...
        var updateResult = @event.UpdateDateTime(command.StartDateTime, command.EndDateTime);
